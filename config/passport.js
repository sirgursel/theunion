 ...
 * Cobot API OAuth2.
 */
passport.use('cobot', new OAuth2Strategy({
  authorizationURL: process.env.COBOT_AUTH_URL,
  tokenURL: process.env.COBOT_TOKEN_URL,
  clientID: process.env.COBOT_CLIENT_ID,
  clientSecret: process.env.COBOT_CLIENT_SECRET,
  callbackURL: process.env.COBOT_REDIRECT_URL,
  passReqToCallback: true
},
(req, accessToken, refreshToken, profile, done) => {
  User.findById(req.user._id, (err, user) => {
    if (err) { return done(err); }
    user.tokens.push({ kind: 'cobot', accessToken });
    user.save((err) => {
      done(err, user);
    });
  });
